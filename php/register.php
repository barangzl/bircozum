...
if ($stmt = $con->prepare('SELECT username, password FROM accounts WHERE username = ?')) {
    ...
    if ($stmt->num_rows > 0) {
      ...
      echo $err_msg;
        ...
        $sql = "INSERT INTO accounts(username, firstName, lastName, password, email) VALUES ('$uname', '$fname', '$lname', md5('$passwrd'), '$email')";
        if(!mysqli_query($con, $sql)) { echo "ERROR: Hush! Sorry $sql. ". mysqli_error($con); }
